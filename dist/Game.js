import { BlackPawn } from "./BlackPiece.js";
import { GameBoard } from "./Board.js";
import PlayerControl from "./PlayerController.js";
import { WhitePawn } from "./WhitePiece.js";
...
class Game {
    static _instance;
    map;
    blackPiecesOnTheBoard = [];
    whitePiecesOnTheBoard = [];
    playerControl;
    isPlayerBlack;
    roomId;
   ...
   */
    updateLocalGameState(data) {
        try {
            ...
            this.blackPiecesOnTheBoard = this.blackPiecesOnTheBoard.filter(piece => data.blackPieces.some((p) => p.x === piece.x && p.y === piece.y));
            this.whitePiecesOnTheBoard = this.whitePiecesOnTheBoard.filter(piece => data.whitePieces.some((p) => p.x === piece.x && p.y === piece.y));
            // Update pieces on the board
            this.blackPiecesOnTheBoard = data.blackPieces.map((pieceData) => {
                const newY = 630 - pieceData.y;
                console.log(pieceData);
                return BlackPieceFactory.make(pieceData.pieceType, pieceData.x, newY, pieceData.firstMove);
            });
            this.whitePiecesOnTheBoard = data.whitePieces.map((pieceData) => {
                const newY = 630 - pieceData.y;
                console.log(pieceData);
